...
describe "GET #index" do
	it "renders the index template and returns status code of 200" do
         expect(response).to have_http_status (200)
      end

	it "assigns @orders to include all orders" do 
            order = FactoryGirl.build_stubbed(:orders)
        expect(assigns(:orders)).to include(orders)
      end
	it "is only accessable through the admin panel"
end

# describe "GET #show" do
# 	it "should render the show template and returns a status code of 200" do 
# 	     expect(response).to have_http_status (200)
# 	end
